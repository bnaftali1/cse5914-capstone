...
def Input_data(es):
    with open("final_dataset.json") as file:
    ...
    for doc in docs:
        ...
        print("successfully for: "+str(i))
        i+=1


def find_match_results(es,column,string_use):
    dsl = {
    'query': {
        'match': {
            column: string_use
...
command=input("Input a command(0 for delete, 1 for create, 2 for input all data, 3 for search, q for quit):")
while(command!='q'):
    ...
    # search the keyword basketball
        ...
        result2=find_match_results(es,'primaryProfession','actor')
